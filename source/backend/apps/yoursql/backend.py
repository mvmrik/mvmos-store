...
def export_multi(conn_id: str, database: str, tables: str, format: str = "sql",
    ...
    if format == "csv":
        ...
        if do_zip:
            # ZIP_STORED (no compression) instead of ZIP_DEFLATED: the deflate
            # compressor buffers internally and won't hand back bytes until
            # the entry closes, which for a big multi-table export means
            # nothing reaches the client for however long the whole export
            # takes — long enough to hit a proxy/browser timeout and land a
            # 0-byte file. Uncompressed writes flush through immediately, so
            # each table's data actually reaches the client as it's produced.
            class _DrainableBuf(_io.RawIOBase):
                def __init__(self):
            ...
            def _zip_stream():
                ...
                with _zf.ZipFile(buf, "w", _zf.ZIP_STORED) as zf:
                    with zf.open(database + ".sql", "w") as entry:
                        entry.write(header.encode())
                        chunk = buf.drain()
                        if chunk:
                            yield chunk
                        for t in table_list:
                            entry.write(_table_sql(t).encode())
                            entry.write(b"\n")
                            chunk = buf.drain()
                            if chunk:
                                yield chunk
                ...
                if tail:
                    yield tail
            return StreamingResponse(_zip_stream(), media_type="application/zip",
                headers={"Content-Disposition": f"attachment; filename={database}.zip"})
        else:
            def _sql_stream():
                yield header.encode()
                for t in table_list:
                    yield _table_sql(t).encode()
                    yield b"\n"
