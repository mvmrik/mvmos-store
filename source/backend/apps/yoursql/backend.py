...
zipstream = _load_sibling("zipstream")
...
def export_multi(conn_id: str, database: str, tables: str, format: str = "sql",
    ...
    if format == "csv":
        ...
        def _dump_chunks():
            """Text chunks for the whole export, MySQL via mysqldump (one
            process for all tables, streams straight from the server) or
            Postgres table-by-table via a server-side cursor — either way,
            rows reach the caller as they're produced instead of the whole
            table being fetched into memory first."""
            yield header
            if dialect is dialect_mysql:
                yield from dialect_mysql.dump_stream(cfg, database, table_list, do_structure, do_data)
                return
            for t in table_list:
                yield f"-- Table: {t}\n"
                if do_structure:
                    ddl = _call(dialect, cfg, "export_table_ddl", database, t)
                    if ddl:
                        yield ddl + "\n\n"
                if do_data:
                    cols, rows = _call(dialect, cfg, "export_rows", database, t)
                    if rows:
                        col_list = ", ".join([dialect.quote_ident(c) for c in cols])
                        qtable = dialect.quote_ident(t)
                        for row in rows:
                            vals = [dialect.escape_literal(row.get(c)) for c in cols]
                            yield f"INSERT INTO {qtable} ({col_list}) VALUES ({', '.join(vals)});\n"
                        yield "\n"

        if do_zip:
            def _entry_chunks():
                for piece in _dump_chunks():
                    yield piece.encode()
            return StreamingResponse(zipstream.stream_zip([(database + ".sql", _entry_chunks())]),
            ...
            def _sql_stream():
                for piece in _dump_chunks():
                    yield piece.encode()
            return StreamingResponse(_sql_stream(), media_type="application/sql",
