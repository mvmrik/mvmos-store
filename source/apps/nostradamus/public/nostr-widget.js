(function(){
  ...
  function mount(root,opts){
    ...
    function renderPendingPill(){
      ...
      var cur=body.querySelector('.nos-newposts-bar'),html=pendingHtml();
    ...
    // "N new posts" means new to the reader, so they open at the top, where the
    // button takes them. Sorting them into the list by time put them wherever
    // their timestamp fell — on Popular a note that just entered the ranking is
    // usually hours old, and a relay can deliver a late note long after it was
    // written — so pressing the button showed the same top as before.
    function acceptPending(){
      quietProfiles={};
      var fresh=pendingItems.filter(function(item){delete pendingKeys[item.key];return!feedKeys[item.key]});
      fresh.sort(function(a,b){return b.at-a.at});
      fresh.forEach(function(item){feedKeys[item.key]=1});
      feedItems=fresh.concat(feedItems);
      if(feedItems.length>200)feedItems.length=200;
      pendingItems=[];
      anchor=null;
      var body=root.querySelector('.nos-body');
    ...
    function noteCardHtml(item,options){
      ...
      var keyAttr=' data-key="'+esc(item.key)+'"';
      if(!note)return'<div class="nos-note'+(options.flat?' nos-flat':'')+'"'+keyAttr+'><div class="nos-empty">'+esc(t('nos_loading'))+'</div></div>';
      ...
      return'<div class="nos-note'+(options.flat?' nos-flat':'')+'"'+keyAttr+(options.clickable===false?'':' data-note="'+esc(note.id)+'"')+'>'+
    ...
    // The reader's place is a note, not a pixel offset. Every repaint rebuilds the
    // list, so images above the note being read start again at no height, and
    // they, videos and link previews keep resizing as they load — a scrollTop
    // kept as a number then pointed somewhere else and the note slid away. So
    // the first note at the top edge is remembered with its distance from that
    // edge, and after a repaint or any size change the scroll is corrected to
    // put it back. At the very top there is nothing to hold on to: that is where
    // the newest notes belong while the list is still arriving.
    var anchor=null,anchorObserver=('ResizeObserver'in window)?new ResizeObserver(function(){
      var body=root.querySelector('.nos-body');if(body)restoreAnchor(body);
    }):null;
    function captureAnchor(body){
      anchor=null;
      if(body.scrollTop<=0)return;
      var top=body.getBoundingClientRect().top,cards=body.querySelectorAll('.nos-note[data-key]');
      for(var i=0;i<cards.length;i++){
        var r=cards[i].getBoundingClientRect();
        if(r.bottom>top){anchor={key:cards[i].dataset.key,offset:r.top-top};return}
      }
    }
    function restoreAnchor(body){
      if(!anchor)return;
      var cards=body.querySelectorAll('.nos-note[data-key]');
      for(var i=0;i<cards.length;i++){
        if(cards[i].dataset.key!==anchor.key)continue;
        var delta=cards[i].getBoundingClientRect().top-body.getBoundingClientRect().top-anchor.offset;
        if(Math.abs(delta)>=1)body.scrollTop+=delta;
        return;
      }
    }
    function watchAnchor(body){
      body.onscroll=function(){captureAnchor(body)};
      if(!anchorObserver)return;
      anchorObserver.disconnect();
      body.querySelectorAll('.nos-note[data-key]').forEach(function(el){anchorObserver.observe(el)});
    }
    function renderBody(){
      var body=root.querySelector('.nos-body');
      if(!body)return;
      if(scrollReset)anchor=null;
    ...
    function pendingHtml(){
      ...
      return'<div class="nos-newposts-bar"><button type="button" class="nos-newposts">'+
        '<span class="nos-newposts-avatars">'+authors.map(function(pk){return avatarHtml(pk)}).join('')+'</span>'+
        esc(t('nos_new_posts',{n:pendingItems.length}))+'</button></div>';
    }
    function renderFeed(body){
      var pending=pendingHtml();
      // The first answer is the list, however it trickles in — relay by relay,
      // or one signature check at a time on Popular. Painting it part-way meant
      // either growing it above the reader or, gated behind the pill, announcing
      // the rest of the very same answer as new posts. So nothing shows until it
      // is complete; "new" is only what arrives after that.
      if(!feedLoaded){
        body.innerHTML=pending+'<div class="nos-empty">'+esc(t('nos_loading'))+'</div>';
        return;
      }
      if(!feedItems.length){
