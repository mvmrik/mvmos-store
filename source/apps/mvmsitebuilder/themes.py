...
_THEMES_DIR = os.path.join(os.path.dirname(__file__), "public", "themes")
...
def install_theme_from_zip(theme_id: str, zip_path: str, owner_id: str) -> dict:
    ...
    _work_root = os.path.join(os.path.dirname(__file__), "work")
    os.makedirs(_work_root, exist_ok=True)
    work_dir = tempfile.mkdtemp(prefix="msb-theme-", dir=_work_root)
    try:
