...
(function () {
  ...
  // Additive per-level stat: value at level 1 plus one step per level after.
  function _lv(type, lvl, key) {
    const c = _bc(type);
    return (c[key] || 0) + (c[key + '_step'] || 0) * (Math.max(1, lvl) - 1);
  }
  ...
  const _keepStore = (lvl) => B.start.store + (B.start.store_step || 0) * (Math.max(1, lvl) - 1);
  ...
  function _cost(type, lvl) {
    const c = _bc(type);
    const l = Math.max(1, lvl);
    const m = Math.pow(B.upgrade_mult, l - 1);
    const out = {};
    ...
    const wood = (base.wood || 0) * m;
    for (const res in (c.tier || {})) {
      if (l >= c.tier[res] && wood) out[res] = (out[res] || 0) + Math.round(wood * B.tier_share[res]);
    }
    return out;
  }
  // What one sortie out of a barracks costs: wooden spears low down, iron once
  // the barracks is worth having.
  function _armCost(lvl) {
    ...
    if (Math.max(1, lvl) >= c.arm_iron_from) out.iron = Math.round(c.arm_iron * n);
    return out;
  }
  ...
  const _fc = () => B.faction;
  // Nothing caps them any more. A camp with a ceiling is a camp the hold
  // outgrows for good, and a game that cannot be lost after the first hour is
  // not a game about lasting. They keep levelling for as long as they can pay
  // for it, and everything they may keep standing grows with that level.
  const _facHuts = (f) => _fc().huts_base + _fc().huts_per_level * (f.lvl - 1);
  const _facBarracks = (f) =>
    Math.floor(_fc().barracks_base + _fc().barracks_per_level * (f.lvl - 1));
  const _facOf = (f, type) => f.buildings.filter(b => b.type === type);
  const _facBuilt = (f, type) => _facOf(f, type).filter(b => b.built >= 1).length;
  // How many men a camp can keep standing: every finished barracks holds more
  // of them at every level, so this is the one number on the map that grows on
  // two axes at once.
  const _facSoldiers = (f) =>
    _facBuilt(f, 'barracks') *
    (_fc().barracks.soldiers + _fc().barracks.soldiers_step * (f.lvl - 1));
  ...
  const _facParty = (f) => Math.max(1, _facSoldiers(f));
  // How long one more man takes. The wait is what a single barracks needs, and
  // every barracks arms its own men — otherwise a camp could raise barracks
  // for ever and still send men at the rate of one, so its parties would grow
  // while its raids grew further apart.
  const _facMuster = (f) =>
    Math.max(_fc().muster_min, _fc().muster + _fc().muster_step * (f.lvl - 1)) /
    Math.max(1, _facBuilt(f, 'barracks'));
  const _facCap = (f) =>
    1 + _facOf(f, 'hut').filter(b => b.built >= 1).length * _fc().hut.workers;
  // What the next level of their keep costs them.
  function _facKeepCost(f) {
    const k = _fc().keep, m = Math.pow(k.mult, f.lvl - 1), out = {};
    for (const res in k.cost) out[res] = Math.round(k.cost[res] * m);
    return out;
  }
  const _facArm = (f) => {
    const out = {};
    for (const res in _fc().arm) out[res] = _fc().arm[res];
    if (f.lvl >= _fc().arm_iron_from) out.iron = (out.iron || 0) + _fc().arm_iron;
    return out;
  };
  ...
  const _facSize = (type) =>
    (type === 'keep' ? _fc().keep.size : type === 'hut' ? _fc().hut.size : _fc().barracks.size);
  ...
  function _facFix(f) {
    ...
    for (const u of f.workers) {
      ...
      if (u.hp == null) u.hp = _fc().worker.hp || 30;
    }
  ...
  function _facPeople(f, dt) {
    ...
    if (f.workers.length < cap) {
      f.spawn = Math.min((f.spawn || 0) + dt, _fc().hut.spawn);
      if (f.spawn >= _fc().hut.spawn) {
        f.spawn = 0;
        f.workers.push({ x: f.x + (Math.random() - 0.5) * 50,
                         y: f.y + (Math.random() - 0.5) * 50,
                         hp: _fc().worker.hp || 30, job: null });
      }
    }
    for (const u of f.workers) {
      ...
      if (u.phase === 'walk') {
        if (_facStep(u, dep, dt, w.speed, _depReach(dep))) { u.phase = 'work'; u.t = 0; }
        continue;
      }
      if (u.phase === 'work') {
        u.t += dt;
        if (u.t >= (_gat(dep.kind).time || 5)) {
          const load = Math.min(w.load, dep.amount);
          _drain(dep, load);
          u.carry = load; u.res = dep.kind; u.phase = 'home';
        }
        continue;
      }
      if (_facStep(u, f, dt, w.speed, _fc().keep.size + 10)) {
  ...
  // Which patch one of their people goes to: the material the camp is shortest
  // of for what it is trying to build next, nearest first, and never so far
  // away that the walk is the whole day.
  function _facPick(f) {
    const want = _facWants(f);
    const reach = _fc().worker.range || 1500;
    for (const res of want) {
      let best = null, bd = Infinity;
      for (const d of _hold.deposits) {
        if (d.kind !== res || d.amount <= 0) continue;
        const dd = _dist(f, d);
        if (dd > reach || dd >= bd) continue;
        bd = dd; best = d;
      }
      if (best) return best;
    }
  ...
  function _facIdle(f, u, dt) {
    ...
    u.rest -= dt;
    _facStep(u, { x: u.wx, y: u.wy }, dt, _fc().worker.speed * 0.5, 6);
  }
  ...
  function _facBuild(f, dt) {
    const site = f.buildings.find(b => b.built < 1);
    if (site) {
      // As many hands as the camp has, so a camp with people up goes up faster.
      const hands = Math.max(1, f.workers.length * 0.5);
      site.built = Math.min(1, site.built + (dt * hands) / (site.build || 14));
      if (site.built >= 1) {
        site.hp = site.maxHp;
        f.stats.built = (f.stats.built || 0) + 1;
        _burst(site.x, site.y, f.color, 10);
      }
      return;
    }
    // Nothing under way: buy the next thing the camp is short of. People
    // before soldiers before rank — a camp with nobody digging never gets to
    // the third of those.
    const c = _fc();
    if (_facOf(f, 'hut').length < _facHuts(f) && _facCanPay(f, c.hut.cost)) {
      _facPay(f, c.hut.cost); _facRaise(f, 'hut'); return;
    }
    if (_facOf(f, 'barracks').length < _facBarracks(f) && _facCanPay(f, c.barracks.cost)) {
      _facPay(f, c.barracks.cost); _facRaise(f, 'barracks'); return;
    }
    ...
    if (_facCanPay(f, cost)) {
      _facPay(f, cost);
      f.lvl += 1;
      const keep = f.buildings[0];
      if (keep) {
        keep.lvl = f.lvl;
        keep.maxHp = c.keep.hp * Math.pow(c.keep.hp_step, f.lvl - 1);
        keep.hp = keep.maxHp;
      }
  ...
  function _facRaise(f, type) {
    ...
    for (let ring = 70; ring <= 260; ring += 34) {
      for (let i = 0; i < 14; i++) {
        ...
        const cfg = type === 'hut' ? c.hut : c.barracks;
        f.buildings.push({
          id: f.next_id++, type: type, x: Math.round(x), y: Math.round(y),
          lvl: 1, built: 0, build: cfg.build,
  ...
  function _facArmy(f, dt) {
    ...
    _facPay(f, arm);
    const s = _fc().soldier;
    const hp = s.hp + s.hp_step * (f.lvl - 1);
    f.army.push({
      x: f.x + (Math.random() - 0.5) * 70, y: f.y + (Math.random() - 0.5) * 70,
      hp: hp, maxHp: hp,
      dmg: s.damage + s.damage_step * (f.lvl - 1),
      speed: (s.speed + s.speed_step * (f.lvl - 1)) * (0.9 + Math.random() * 0.2),
  ...
  function _facWar(f, dt) {
    ...
    if (f.grace > 0) return;
    const party = _facParty(f);
    if (f.army.length < party) { f.ready = 0; return; }
    ...
    const keep = _keep();
    const going = f.army.splice(0, party);
  ...
  function _buildings(dt) {
    // The keep houses its own person, so a hold always has someone to build the
    // first house — and if the last person is killed, someone moves back in.
    ...
    for (const b of _hold.buildings) {
      ...
      if (b.type === 'workshop') {
        ...
        if (room > 0) {
          const per  = _bc('workshop').iron_per_ammo || 0;
  ...
  function _take(w, kind) {
    ...
    if (kind === 'mine') {
      ...
      // A patch somebody is already working is the last choice, so the hold
      // spreads over every deposit it can reach rather than queueing at one —
      // and how far they are willing to walk is the only thing stopping them,
      // which is what makes a rich seam on the far side of the map a decision.
      for (const res of _byNeed()) {
        const all = _hold.deposits.filter(
          d => d.kind === res && d.amount > 0 && d.id !== w._skipDep);
        if (!all.length) continue;
        const free = all.filter(d => !_hold.workers.some(
          x => x !== w && x.job === 'mine' && x.dep === d.id));
        const q = _nearest(w, free.length ? free : all);
        if (q) {
  ...
  function _towers(dt) {
    for (const b of _hold.buildings) {
      ...
      // One volley costs one round however many barrels the tower has grown —
      // the magazine is about how long it can fight, not how hard.
      _spend(b, 1);
      b._rl = _reload(b.lvl);
  ...
  function _spawnDeposit(kind) {
    ...
    for (let n = 0; n < 200; n++) {
      ...
      // Mostly out in the middle ground, which is where the map wants the
      // fighting to be; sometimes close in to somebody, so a corner that has
      // eaten everything near it is not dead ground for the rest of the game.
      const site = sites[Math.floor(Math.random() * sites.length)];
      const wild = Math.random() < 0.55;
      const dist = wild ? d.wild_near + Math.random() * (d.wild_far - d.wild_near)
                        : d.near + Math.random() * (d.far - d.near);
      const a    = Math.random() * Math.PI * 2;
      const x    = site.x + Math.cos(a) * dist;
      const y    = site.y + Math.sin(a) * dist;
      if (!(x > r + 20 && x < W() - r - 20 && y > r + 20 && y < W() - r - 20)) continue;
      // Never on top of anybody's hold — theirs included, or a camp wakes up
      // with a forest through its barracks.
      if (sites.some(s => Math.hypot(s.x - x, s.y - y) < r + 120)) continue;
  ...
  function _canPlace(type, x, y) {
    ...
    if (!_canPay(_cost(type, 1))) return 'sh_cant_afford';
    return null;
  }
  ...
  // The first and only free building: the keep, and the person who lives in it.
  ...
  function _found(x, y) {
    ...
    _hold.workers.push({ x: x + 18, y: y + 18, hp: B.worker.hp, job: null });
  ...
  function _place(type, x, y) {
    ...
    if (why) return _say(_t(why));
    const cost = _cost(type, 1);
    _pay(cost);
    _hold.buildings.push({
      id: _hold.next_id++, type: type, x: x, y: y, lvl: 1,
      hp: _maxHp(type, 1), built: 0, ammo: 0, _rl: 0, _sp: 0,
    });
  ...
  function _hintKey() {
    ...
    const dry = bs.some(b => b.type === 'tower' && !b.keep && b.built >= 1 && (b.ammo || 0) < 1);
  ...
  function _markAffordable() {
    ...
    _bar.querySelectorAll('[data-build]').forEach(el => {
      const cost = _cost(el.dataset.build, 1);
  ...
  function _statList(type, l, b) {
    ...
    if (type === 'tower') {
      ...
      row(_t('sh_stat_people'), _lv('house', l, 'workers'));
      row(_t('sh_stat_moves_in'), Math.round(_bc('house').spawn) + 's');
      ...
      row(_t('sh_stat_ammo_cost'), RES_GLYPH.iron + ' ' + per);
  ...
  function _previewPanel(type) {
    ...
    const cost = found ? null : _cost(type, 1);
    panel.innerHTML =
  ...
  function _renderFacPanel() {
    ...
    const rows =
      ...
      _statRow(_t('sh_fac_huts'), _facOf(f, 'hut').length + ' / ' + _facHuts(f)) +
      _statRow(_t('sh_fac_barracks'), _facOf(f, 'barracks').length + ' / ' + _facBarracks(f)) +
      _statRow(_t('sh_fac_army'), f.army.length + ' / ' + party) +
    ...
    const note = f.grace > 0 ? _t('sh_fac_quiet', { n: Math.ceil(f.grace) })
               ...
               : f.army.length >= party ? _t('sh_fac_ready')
               : _t('sh_fac_growing');
  ...
  function _renderStrip() {
    ...
    const cost = found ? null : _cost(_placing, 1);
  ...
  function _syncStrip() {
    ...
    const cost = _placing === 'keep' ? null : _cost(_placing, 1);
  ...
  function _drawFactions(v) {
    for (const f of _hold.factions) {
      ...
      for (const u of f.workers) {
        _mark(u.x, u.y, 11, f.color, 0.85);
        _g.font = '17px serif';
        _g.fillText('🧑', u.x, u.y + 1);
        _g.font = '11px serif';
        ...
        if (u._lock) _g.fillText('⚔', u.x, u.y - 13);
        else if (u.carry) _g.fillText(RES_GLYPH[u.res] || '⛏', u.x, u.y - 13);
        if (u.hp != null && u.hp < (B.faction.worker.hp || 30)) {
          _g.fillStyle = 'rgba(0,0,0,.45)'; _g.fillRect(u.x - 8, u.y - 20, 16, 3);
          _g.fillStyle = f.color;
          _g.fillRect(u.x - 8, u.y - 20, 16 * Math.max(0, u.hp / (B.faction.worker.hp || 30)), 3);
        }
      }
      ...
      for (const s of f.army) {
        _mark(s.x, s.y, 12, f.color, 0.9);
        _g.font = '18px serif';
        _g.fillText('👹', s.x, s.y + 1);
  ...
  function _draw() {
    ...
    if (_selDep && _hold.deposits.includes(_selDep)) {
      _ring(_selDep.x, _selDep.y, _depR(_selDep) + 10, green, 0.6);
    }
    ...
    for (const w of _hold.workers) {
      // Your own people stand on your own colour, the same way theirs do: with
      // four camps walking about the map, "mine or theirs" has to be one look.
      _mark(w.x, w.y, 12, acc, 0.8);
      _g.font = '19px serif';
      _g.fillText('🧑', w.x, w.y + 1);
      // What they are doing, above their head — the priority made visible.
      // A miner carries the material's own mark, so which of the three the hold
      // is short of can be read off the map without opening anything.
      _g.font = '12px serif';
      let jg = JOB_GLYPH[w.job] || JOB_GLYPH.idle;
      if (w.job === 'mine') {
        const dep = _depById(w.dep);
        jg = RES_GLYPH[(dep && dep.kind) || w.res] || jg;
      }
      _g.fillText(jg, w.x, w.y - 14);
      if (w.carry) {
        _g.fillStyle = green;
        _g.fillRect(w.x - 3, w.y + 12, 6, 3);
      }
    ...
    // figure the workers are drawn as, standing on a marked patch so his side
    // is never in doubt, carrying what he is doing over his head — a shield
    // while he walks his rounds, crossed blades once he is in a fight.
    for (const s2 of _hold.soldiers) {
      _mark(s2.x, s2.y, 12, green, 0.85);
      _g.font = '19px serif';
      _g.fillText('🧑', s2.x, s2.y + 1);
      _g.font = '12px serif';
    ...
    for (const r of _raiders) {
      // A raider stands on his own camp's colour, so a hold being hit from two
      // corners at once can tell which of them is which.
      const f = _facById(r.fx);
      _mark(r.x, r.y, 13, (f && f.color) || red, 0.9);
      _g.font = '21px serif';
      _g.fillText('👹', r.x, r.y + 1);
  ...
  // The patch a person stands on, in the colour of whoever they belong to.
  // Everybody on the map is drawn as the same little figure, and an emoji comes
  // in its own colours whatever the fill is set to — so the side is said by the
  // ground under the feet, which also stays readable when the figure itself is
  // a few pixels across.
  function _mark(x, y, r, color, alpha) {
    const a = alpha == null ? 0.85 : alpha;
    // A ring rather than a plain disc: the figure is drawn over the middle of
    // this, so a disc alone would end up hidden behind it. The ring stands
    // outside the figure, and at the zoom where the whole map is on screen the
    // ring is all there is left to see — which is the point.
    _g.globalAlpha = a * 0.55;
    _g.fillStyle = color;
    _g.beginPath(); _g.arc(x, y + 1, r, 0, Math.PI * 2); _g.fill();
    _g.globalAlpha = a;
    _g.strokeStyle = color; _g.lineWidth = 2.5;
    _g.beginPath(); _g.arc(x, y + 1, r, 0, Math.PI * 2); _g.stroke();
    _g.globalAlpha = 1;
  }
  ...
  function _drawBuilding(b, acc, green, red) {
    ...
    if (b.hp < maxHp) {
      ...
      _g.fillStyle = b.hp / maxHp > 0.4 ? green : red;
      _g.fillRect(b.x - r, b.y - r - 10, r * 2 * Math.max(0, b.hp / maxHp), 4);
    }
    if (b.type === 'tower' && done) {
      ...
      _g.textAlign = 'center'; _g.textBaseline = 'middle';
      _g.fillStyle = have ? _css('--yellow', '#f9e2af') : red;
