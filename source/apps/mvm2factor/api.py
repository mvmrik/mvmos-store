...
Routes are declared on desktop_router and mounted at /api/apps/mvm2factor
behind the desktop session — the same address backend.py used, so main.js is
unchanged. Accounts stay scoped to session["effective_user"] as before.
"""
...
def list_accounts(session=Depends(current_session)):
    with _conn() as c:
        rows = c.execute(
...
def add_account(data: AccountIn, session=Depends(current_session)):
...
@desktop_router.delete("/accounts/{aid}")
def delete_account(aid: str, session=Depends(current_session)):
    with _conn() as c:
...
@desktop_router.post("/accounts/{aid}/use")
def mark_used(aid: str, session=Depends(current_session)):
    with _conn() as c:
        c.execute(
...
def get_prefs(session=Depends(current_session)):
    with _conn() as c:
...
def set_prefs(data: PrefsIn, session=Depends(current_session)):
