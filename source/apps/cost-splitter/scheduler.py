...
def _get_core_settings(db_path):
    """currency/date_format come from the platform API — an app has no
    business opening the core DB itself. Same process, so this is a plain
    function call. db_path stays in the signature so callers are unchanged."""
    try:
        return sys.modules["backend.platform_api"].get_settings()
    except Exception:
        return {}
