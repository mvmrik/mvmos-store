...
GM.repoView = (function() {
  ...
  async function _loadStatus(container, repo) {
    ...
    try {
      ...
      if (!s.files.length) {
        tc.innerHTML = `
          <div style="color:var(--text-dim);font-size:.82rem;opacity:.7;padding:20px 0;text-align:center">Nothing to commit — working tree clean</div>
          ${_commitArea(repo, true)}
        `;
      } else {
        tc.innerHTML = `
          <div style="font-size:.75rem;color:var(--text-dim);margin-bottom:8px">${s.files.length} changed file${s.files.length !== 1 ? 's' : ''}</div>
