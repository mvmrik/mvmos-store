(function(){
  ...
  var API='/pub/bwallett',ITERATIONS=600000,MIN_PASSWORD=10,SATS=100000000,SYNC_RETRY_DELAYS=[5000,15000,30000];
  ...
  function mount(root,opts){
    ...
    var state={remote:null,key:null,doc:null,view:'home',sync:null,fees:null,policy:{min_send_sats:0,max_send_sats:0},busy:false,destroyed:false,syncError:false,syncRetries:0};
    ...
    function clearSyncRetry(){if(syncRetryTimer){clearTimeout(syncRetryTimer);syncRetryTimer=null}}
    async function refreshWallet(){
      if(!state.doc||state.busy)return;clearSyncRetry();state.busy=true;renderApp();
      try{
        ...
        state.sync=results[0];state.fees=results[1];state.syncError=false;state.syncRetries=0;
        ...
        state.busy=false;state.syncError=true;renderApp();toast(t('tw_network_error'));
        if(state.syncRetries<SYNC_RETRY_DELAYS.length){
          var delay=SYNC_RETRY_DELAYS[state.syncRetries];state.syncRetries++;
          syncRetryTimer=setTimeout(function(){syncRetryTimer=null;if(!state.destroyed)refreshWallet()},delay);
        }
      }
    }
    ...
    function renderApp(){
      ...
      shell.innerHTML='<header class="bw-header"><div class="bw-brand"><span class="bw-logo">₿</span><span>BwalletT</span></div><div class="bw-head-actions"><button class="bw-icon refresh'+(state.syncError?' sync-error':'')+'" title="'+esc(state.syncError?t('tw_sync_failed'):t('tw_refresh'))+'">↻</button><button class="bw-icon settings" title="'+esc(t('tw_settings'))+'">⚙</button></div></header><main class="bw-main"></main><nav class="bw-nav">'+nav.map(function(v){return'<button data-view="'+v+'" class="'+(state.view===v?'active':'')+'">'+esc(t('tw_'+v))+'</button>'}).join('')+'</nav>';
      shell.querySelector('.bw-brand').onclick=function(){state.view='home';renderApp()};shell.querySelector('.refresh').onclick=function(){state.syncRetries=0;refreshWallet()};shell.querySelector('.settings').onclick=function(){state.view='settings';renderApp()};
    ...
    function lock(){clearSyncRetry();state.key=null;state.doc=null;state.sync=null;state.fees=null;state.syncError=false;state.syncRetries=0;state.view='home';renderGate()}
    load();
    return{destroy:function(){state.destroyed=true;clearSyncRetry();state.key=null;state.doc=null;root.innerHTML=''}};
  }
