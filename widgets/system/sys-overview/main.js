// mvmOS Widget: System Overview v1.3.0
const _sysOv18n = {
  en: { label: 'System Overview', title: 'SYSTEM', uptime: 'Uptime', host: 'Host' },
  bg: { label: 'Системен преглед', title: 'СИСТЕМА', uptime: 'Работно време', host: 'Хост' },
};
function _sysOvt(key) { const lang = window.mvmOS?.lang || 'en'; return (_sysOv18n[lang] || _sysOv18n.en)[key] || key; }
    ...
    function render() {
      container.innerHTML = `
        <div style="width:220px;background:var(--surface,rgba(17,17,27,.88));border:1px solid var(--border,rgba(255,255,255,.08));border-radius:10px;padding:14px;box-shadow:0 8px 32px rgba(0,0,0,.5)">
          <div style="font-size:.72rem;font-weight:700;color:var(--text,#cdd6f4);letter-spacing:.06em;margin-bottom:12px">${_sysOvt('title')}</div>
          ...
          <div style="border-top:1px solid var(--border,rgba(255,255,255,.06));margin-top:4px;padding-top:8px;display:flex;justify-content:space-between">
            <span style="font-size:.68rem;color:var(--text-dim,#585b70)">${_sysOvt('uptime')}</span>
            <span id="so-uptime" style="font-size:.68rem;color:var(--text,#a6adc8)">—</span>
          </div>
          <div style="display:flex;justify-content:space-between;margin-top:4px">
            <span style="font-size:.68rem;color:var(--text-dim,#585b70)">${_sysOvt('host')}</span>
